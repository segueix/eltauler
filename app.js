...
let openingErrorTapSelectedSquare = null;
let openingPracticeTapSelectedSquare = null;
let openingErrorTapMoveEnabled = false;
let openingPracticeTapMoveEnabled = false;
let openingErrorLastTapEventTs = 0;
let openingPracticeLastTapEventTs = 0;
...
function resizeOpeningBoardsToViewport() {
    const openingScreen = document.getElementById('opening-screen');
    if (!openingScreen) return;
    const isVisible = (openingScreen.style.display !== 'none') && (openingScreen.offsetParent !== null);
    if (!isVisible) return;

    ['opening-error-board', 'opening-practice-board'].forEach((id) => {
        ...
        if (id === 'opening-error-board' && openingErrorBoard && typeof openingErrorBoard.resize === 'function') {
            openingErrorBoard.resize();
        }
        if (id === 'opening-practice-board' && openingPracticeBoard && typeof openingPracticeBoard.resize === 'function') {
...
function clearOpeningErrorTapSelection() {
    openingErrorTapSelectedSquare = null;
    $('#opening-error-board .square-55d63').removeClass('tap-selected tap-move');
}

function clearOpeningPracticeTapSelection() {
    openingPracticeTapSelectedSquare = null;
    $('#opening-practice-board .square-55d63').removeClass('tap-selected tap-move');
}

function highlightOpeningErrorTapSelection(square) {
    $('#opening-error-board .square-55d63').removeClass('tap-selected tap-move');
    if (!square || !openingErrorGame) return;
    const sel = $(`#opening-error-board .square-55d63[data-square='${square}']`);
    sel.addClass('tap-selected');

    const moves = openingErrorGame.moves({ square: square, verbose: true });
    for (const mv of moves) {
        $(`#opening-error-board .square-55d63[data-square='${mv.to}']`).addClass('tap-move');
    }
}
...
function highlightOpeningPracticeTapSelection(square) {
    ...
    const sel = $(`#opening-practice-board .square-55d63[data-square='${square}']`);
    sel.addClass('tap-selected');

    const moves = openingPracticeGame.moves({ square: square, verbose: true });
    for (const mv of moves) {
        $(`#opening-practice-board .square-55d63[data-square='${mv.to}']`).addClass('tap-move');
    }
}
...
function enableOpeningErrorTapToMove() {
    if (openingErrorTapMoveEnabled) return;
    openingErrorTapMoveEnabled = true;
    const boardEl = document.getElementById('opening-error-board');
    if (boardEl) boardEl.style.touchAction = 'none';

    $('#opening-error-board').off('.tapmove')
        .on(`pointerdown.tapmove touchstart.tapmove`, '.square-55d63', function(e) {
            if (!openingErrorGame || openingErrorGame.game_over() || !isOpeningErrorSession) return;

            if (e && e.preventDefault) e.preventDefault();

            const nowTs = Date.now();
            if (nowTs - openingErrorLastTapEventTs < 180) return;
            openingErrorLastTapEventTs = nowTs;

            const square = $(this).attr('data-square');
            if (!square) return;

            if (!openingErrorTapSelectedSquare) {
                const p = openingErrorGame.get(square);
                if (!p || p.color !== openingErrorGame.turn()) return;
                openingErrorTapSelectedSquare = square;
                highlightOpeningErrorTapSelection(square);
                return;
            }

            if (square === openingErrorTapSelectedSquare) {
                clearOpeningErrorTapSelection();
                return;
            }

            const result = onOpeningErrorDrop(openingErrorTapSelectedSquare, square);
            if (result !== 'snapback') {
            ...
            const p2 = openingErrorGame.get(square);
            if (p2 && p2.color === openingErrorGame.turn()) {
                openingErrorTapSelectedSquare = square;
                highlightOpeningErrorTapSelection(square);
            }
        });
...
function disableOpeningErrorTapToMove() {
    if (!openingErrorTapMoveEnabled) return;
    openingErrorTapMoveEnabled = false;
    $('#opening-error-board').off('.tapmove');
    const boardEl = document.getElementById('opening-error-board');
    if (boardEl && controlMode !== 'drag') boardEl.style.touchAction = '';
    clearOpeningErrorTapSelection();
...
function enableOpeningPracticeTapToMove() {
    if (openingPracticeTapMoveEnabled) return;
    openingPracticeTapMoveEnabled = true;
    const boardEl = document.getElementById('opening-practice-board');
    if (boardEl) boardEl.style.touchAction = 'none';

    $('#opening-practice-board').off('.tapmove')
        .on(`pointerdown.tapmove touchstart.tapmove`, '.square-55d63', function(e) {
            if (!openingPracticeGame || openingPracticeGame.game_over()) return;
            if (openingPracticeMoveCount >= 10) return;
            if (openingPracticeGame.turn() !== openingPracticeColor) return;

            if (e && e.preventDefault) e.preventDefault();

            const nowTs = Date.now();
            if (nowTs - openingPracticeLastTapEventTs < 180) return;
            openingPracticeLastTapEventTs = nowTs;

            const square = $(this).attr('data-square');
            if (!square) return;

            if (!openingPracticeTapSelectedSquare) {
                const p = openingPracticeGame.get(square);
                if (!p || p.color !== openingPracticeGame.turn()) return;
                openingPracticeTapSelectedSquare = square;
                highlightOpeningPracticeTapSelection(square);
                return;
            }

            if (square === openingPracticeTapSelectedSquare) {
                clearOpeningPracticeTapSelection();
                return;
            }

            const result = onOpeningPracticeDrop(openingPracticeTapSelectedSquare, square);
            if (result !== 'snapback') {
...
function disableOpeningPracticeTapToMove() {
    if (!openingPracticeTapMoveEnabled) return;
    openingPracticeTapMoveEnabled = false;
    $('#opening-practice-board').off('.tapmove');
    const boardEl = document.getElementById('opening-practice-board');
    if (boardEl && controlMode !== 'drag') boardEl.style.touchAction = '';
    clearOpeningPracticeTapSelection();
...
function updateOpeningBoardInteractivity() {
    ...
    if (openingErrorBoard) {
        openingErrorBoard.draggable = !shouldUseTap;
        if (shouldUseTap) {
            enableOpeningErrorTapToMove();
        } else {
            disableOpeningErrorTapToMove();
        }
    }

    if (openingPracticeBoard) {
        openingPracticeBoard.draggable = !shouldUseTap;
        if (shouldUseTap) {
